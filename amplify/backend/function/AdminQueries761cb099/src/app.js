/* Amplify Params - DO NOT EDIT
	AUTH_EEADMINAPPEE8AF8D2_USERPOOLID
	ENV
	REGION
...
const allowedGroup1 = process.env.GROUP1;
...
const checkGroup = function (req, res, next) {
  ...
  if (req.apiGateway.event.requestContext.authorizer.claims['cognito:groups']) {
    ...
    if (!(allowedGroup && (groups.indexOf(allowedGroup) > -1 || groups.indexOf(allowedGroup1) > -1))) {
...
app.post('/updateUser', async (req, res, next) => {
  ...
  try {
    const response = await updateUser(req.body.userId, req.body.email);
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
...
app.post('/deleteUser', async (req, res, next) => {
  ...
  try {
    const userResponse = await deleteUser(req.body.userId);
